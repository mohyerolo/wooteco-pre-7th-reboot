...
[4. 네 번째 미션: 편의점](#네-번째-미션-편의점)  
[4-1. 처음 구현했던 것과 달라진 점](#처음-구현했던-것과-달라진-점-2)
...
[4주 차 미션 진행 깃허브](https://github.com/mohyerolo/java-convenience-store-7-mohyerolo)
...
- 사용자가 입력한 상품의 가격과 수량을 기반으로 최종 결제 금액을 계산한다.
    - 총구매액 = 상품별 가격 * 수량
        - 최종 결제 금액: 총구매액 - (프로모션 및 멤버십 할인 금액)
...
1. 도메인을 DTO로 변환하는 용도로 사용했던 서비스를 도메인간의 흐름 관리와 비즈니스 로직 흐름 관리로 사용
    1.  서비스 이용으로 객체지향적 설계와 캡슐화 사이의 협력 균형을 가지려고 노력함
2. 주문 아이템에서 필요없던 계산 로직을 서비스를 사용해 도메인 복잡성 감소
3. 도메인이 자신과 관련이 있는 메서드만을 가지도록 노력
4. 프로모션이 없으면 null이 아닌 NoPromotion이라는 Promotion 상속 클래스 사용
